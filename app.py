...
def initialize_agent(_retriever):
    ...
    try:
        ...
        if _retriever:
            ...
            st.warning(
                "Retriever tool (RBI document search) is unavailable.", icon="⚠️"
            )
...
with st.sidebar:
    ...
    st.info(
        "Upload relevant RBI PDFs (Master Directions, Circulars, etc.) here. Reload required after adding/changing files."
    )
        ...
        if st.session_state.current_upload_filename != uploaded_file.name:
            with st.spinner("Processing uploaded document..."):
                ...
                if doc_text:
                    ...
                    st.success(
                        f"✅ Ready to analyze '{uploaded_file.name}'. Ask a question about it in the chat."
                    )
                    st.info(
                        f"Document content (limited to {MAX_UPLOAD_TEXT_CHARS} chars) will be added as context for your next query."
                    )
                else:
            ...
            st.success(
                f"✅ Document '{st.session_state.current_upload_filename}' is loaded. Ask a question about it."
            )
...
if prompt := st.chat_input("Ask about RBI rules, analyze uploaded doc, check compliance..."):
    ...
    if user_context:
        ...
        context_display_message = (
            "ℹ️ Querying with context from the uploaded document."
        )
    else:
        context_info_for_prompt = (
            "No document context provided by user."  # Be explicit in prompt context
        )
    ...
    with st.chat_message("assistant", avatar="🤖"):
        if (
            context_display_message
        ):  # Display context usage message only if context is present
            st.info(context_display_message)

        st_callback_container = (
            st.container()
        )  # Define container where callbacks will render
